...
void v_add(double* x, double* y, double* z) {
	...
	#pragma omp parallel
	{
		...
		for (int i=tid*ARRAY_SIZE/nthreads; i<ARRAY_SIZE && i<(tid+1)*ARRAY_SIZE/nthreads; i++){
			z[i]=x[i]+y[i];
		}
	/*
		//first method
		nthreads = omp_get_num_threads();
		tid = omp_get_thread_num();
		for (int i = tid; i<ARRAY_SIZE;i+=nthreads){
				z[i] = x[i] + y[i];
		}
		*/
...
int verify(double* x, double* y) {
	...
	for(int i=0; i<ARRAY_SIZE; i++)
		if(z_oracle[i] != z_v_add[i]){
			//printf("%dth, x:%f, y:%f, %f, %f\n", i,*(x+i), *(y+i),z_oracle[i],z_v_add[i]);
			return 0;
		}	
	return 1;
}
